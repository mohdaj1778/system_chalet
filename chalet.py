...
with tab1:
    ...
    if user_pass == "4321":
        ...
        # جلب الأوقات الافتراضية بناءً على الشفت المختار
        default_from = SHIFTS[selected_shift]["from"]
        default_to = SHIFTS[selected_shift]["to"]
        
        # صناديق اختيار الوقت (تتغير تلقائياً ومفتوحة للتعديل اليدوي في نفس الوقت)
        col1, col2 = st.columns(2)
        with col1:
            time_from = st.time_input("⏳ وقت الوصول المعتمد (يمكنك تعديله):", value=default_from, key="time_from_u")
        with col2:
            time_to = st.time_input("⏳ وقت الخروج المعتمد (يمكنك تعديله):", value=default_to, key="time_to_u")
        ...
        if submit_button:
            if not customer_name:
                ...
                new_data = {
                    ...
                    "وقت الوصول": time_from.strftime('%H:%M'),
                    "وقت الخروج": time_to.strftime('%H:%M'),
                    "المبلغ": amount
                }
        ...
        st.markdown("---")
        st.subheader("📊 جدول الحجوزات والمالية الكامل")
        if df_bookings.empty:
...
with tab2:
    ...
    if owner_pass == "1234":
        ...
        # جلب الأوقات الافتراضية لشفت المالك
        default_from_o = SHIFTS[selected_shift_o]["from"]
        default_to_o = SHIFTS[selected_shift_o]["to"]
        
        # صناديق اختيار الوقت للمالك (تتغير تلقائياً ومفتوحة للتعديل اليدوي أيضاً)
        col1, col2 = st.columns(2)
        with col1:
            time_from_o = st.time_input("⏳ وقت الوصول المعتمد (يمكنك تعديله):", value=default_from_o, key="time_from_o")
        with col2:
            time_to_o = st.time_input("⏳ وقت الخروج المعتمد (يمكنك تعديله):", value=default_to_o, key="time_to_o")
        ...
        if submit_button_o:
            if is_already_booked(date_str_o, selected_shift_o):
                ...
                new_data_o = {
                    ...
                    "وقت الوصول": time_from_o.strftime('%H:%M'),
                    "وقت الخروج": time_to_o.strftime('%H:%M'),
                    "المبلغ": 0
                }
        ...
        st.markdown("---")
        st.subheader("📊 تقويم الحجوزات العام (رؤية المالك الآمنة)")
        if df_bookings.empty:
